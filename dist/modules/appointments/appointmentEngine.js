...
export class AppointmentEngine {
     ...
     */
    static async bookAppointment(input) {
        ...
        if (!service)
            throw new AppError('Service not found for this business', 404);
        if (!service.active)
            throw new AppError('Selected service is inactive', 400);
     ...
     */
    static async rescheduleAppointment(input) {
     ...
     */
    static async cancelAppointment(input) {
     ...
     */
    static async updateStatus(businessId, appointmentId, status, changedBy = 'system', reason) {
        const existing = await prisma.appointment.findFirst({
            where: { id: appointmentId, businessId },
        });
        if (!existing)
